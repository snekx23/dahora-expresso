...
test('Realtime Mapa da Frota — Auditoria de Código e Lógica de Atualização', async (t) => {
  ...
  await t.test('4. Evento DELETE na tabela fleet remove do mapa e do dicionário ownerFleetMarkers', () => {
    assert.ok(appJs.includes("ownerFleetMarkers[riderId].setMap(null)"), 'DELETE deve remover a bolinha do mapa com setMap(null)');
    assert.ok(appJs.includes("delete ownerFleetMarkers[riderId]"), 'DELETE deve excluir o registro de ownerFleetMarkers');
  });
