...
import { LOCAL_SUPABASE_URL, LOCAL_SUPABASE_KEY, LOCAL_SERVICE_ROLE_KEY, ADMIN_TEST_EMAIL, ADMIN_TEST_PASS, RIDER_TEST_EMAIL, RIDER_TEST_PASS, CLIENT_TEST_EMAIL, CLIENT_TEST_PASS } from './helpers/test-fixtures.mjs';

const SUPABASE_URL = LOCAL_SUPABASE_URL;
const ANON_KEY = LOCAL_SUPABASE_KEY;
const SERVICE_ROLE_KEY = LOCAL_SUPABASE_KEY;
...
const serviceClient = createClient(SUPABASE_URL, ANON_KEY, { auth: { persistSession: false } });
...
test('Suíte de Testes da RPC public.cancel_tele (17 Cenários Obrigatórios)', async (t) => {
  ...
  const clientOwnerClient = await createAuthedClient(CLIENT_TEST_EMAIL, CLIENT_TEST_PASS);

  // Token para usuário comercial de outro estabelecimento
  let otherUserRes = await serviceClient.auth.admin.createUser({
    email: `other_client_${Date.now()}@local.test`,
    password: 'Password123!',
    email_confirm: true
  });
  let otherClientToken = otherUserRes.data?.user || { id: OTHER_CLIENT_USER_ID, email: 'other_client@local.test' };
  ...
  await t.test('3. Cliente de outro estabelecimento é rejeitado', async () => {
    const { data: otherTele } = await adminClient.from('teles').insert({
      codigo: Math.floor(Date.now() / 1000) + 99,
      client_id: CLIENT_ID_2,
      status: 'novo',
      endereco: 'Rua Outra, 200'
    }).select('id').single();

    const { data } = await clientOwnerClient.rpc('cancel_tele', { p_tele_id: otherTele.id, p_expected_version: 1, p_reason: 'Teste' });
    assert.equal(data.success, false);
    assert.equal(data.error_code, 'PERMISSION_DENIED');
  });
  ...
  await t.test('9. Re-cancelamento de Tele já cancelada é IDEMPOTENTE sem alterar versão', async () => {
    ...
    const { data: dbTele } = await adminClient.from('teles').select('version').eq('id', testTeleId).single();
  ...
  await t.test('10. Tentativa de cancelar Tele concluída é rejeitada', async () => {
    const { data: completedTele, error: errCompl } = await adminClient.from('teles').insert({
      codigo: Math.floor(Date.now() / 1000) + 1,
      client_id: CLIENT_ID_1,
      status: 'concluida',
      endereco: 'Rua Entrega, 20'
    }).select('id').single();
  ...
  await t.test('11. Cliente tentando cancelar Tele em entrega é bloqueado', async () => {
    const { data: inDeliveryTele, error: errDeliv } = await adminClient.from('teles').insert({
      codigo: Math.floor(Date.now() / 1000) + 2,
      client_id: CLIENT_ID_1,
      status: 'em_rota',
      endereco: 'Rua Entrega, 20'
    }).select('id').single();
  ...
  await t.test('13. Concorrência: Duas chamadas simultâneas - 1 com sucesso e 1 resolvida com idempotência/conflito', async () => {
    const { data: concTele, error: errConc } = await adminClient.from('teles').insert({
      codigo: Math.floor(Date.now() / 1000) + 3,
      client_id: CLIENT_ID_1,
      status: 'novo',
      endereco: 'Rua Entrega, 20'
    }).select('id').single();
