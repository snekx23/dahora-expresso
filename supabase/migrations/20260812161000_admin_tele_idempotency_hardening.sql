-- =====================================================================
-- Dahora Expresso — Migration: Idempotência Estrita na Criação de Tele Admin
-- Timestamp: 20260812161000
-- Descrição: Adiciona índice UNIQUE parcial em public.teles para (client_id, admin_request_idempotency_key)
--            e atualiza public.create_admin_tele para executar verificação prévia síncrona,
--            garantindo retorno idempotente ('is_idempotent': true, 'idempotent_replay': true) sem
--            duplicação de registros, sequências ou eventos de auditoria sob concorrência simultânea.
-- =====================================================================

-- 1. Garantia de Unicidade no Banco (Índice UNIQUE Parcial por Cliente e Chave de Idempotência)
CREATE UNIQUE INDEX IF NOT EXISTS idx_teles_client_admin_idempotency_key
ON public.teles (client_id, admin_request_idempotency_key)
WHERE admin_request_idempotency_key IS NOT NULL AND pg_catalog.btrim(admin_request_idempotency_key) <> '';

-- 2. Atualizar RPC public.create_admin_tele com Trativa de Idempotência Síncrona e Replay Atômico
...
DECLARE
  ...
  v_pickup_establishment_name TEXT;
...
BEGIN
  ...
  -- Derivar o Snapshot Autoritativo do Nome do Estabelecimento diretamente do commercial_client
  v_pickup_establishment_name := NULLIF(pg_catalog.btrim(v_client.establishment_name), '');
  ...
  -- 7. Idempotência (Verificação Prévia Síncrona)
  ...
  -- Replay de Idempotência: Checar se a Tele já foi criada para o mesmo cliente e chave
  ...
  -- 12. Inserção Atômica com Proteção contra Concorrência Simultânea
  BEGIN
    INSERT INTO public.teles (
      ...
      pickup_latitude, pickup_longitude, place_id, delivery_state, pickup_place_id, pickup_establishment_name
    ) VALUES (
      ...
      v_pickup_lat_final, v_pickup_lng_final, p_place_id, COALESCE(p_delivery_state, 'RS'), v_pickup_place_id_final, v_pickup_establishment_name
    )
  ...
  -- 13. Registros de evento e auditoria (Apenas para PRIMEIRA CRIAÇÃO)
