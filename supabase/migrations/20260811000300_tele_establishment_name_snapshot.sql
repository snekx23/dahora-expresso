-- =====================================================================
-- Dahora Expresso — Migration: Snapshot Autoritativo do Nome do Estabelecimento na Tele
-- Timestamp: 20260811000300
-- Descrição: Adiciona pickup_establishment_name em public.teles e atualiza RPCs (create_admin_tele e create_client_tele)
--            para gravar autoritativamente o nome do cliente no momento da criação.
-- =====================================================================

-- 1. Adicionar coluna pickup_establishment_name em public.teles se ainda não existir
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns 
    WHERE table_schema = 'public' AND table_name = 'teles' AND column_name = 'pickup_establishment_name'
  ) THEN
    ALTER TABLE public.teles ADD COLUMN pickup_establishment_name TEXT;
  END IF;
END $$;
...
BEGIN
  ...
  -- 12. Inserção Atômica Persistindo o Snapshot Imutável de Coleta e Nome do Estabelecimento
...
BEGIN
  ...
  -- 10. Inserir Tele em aguardando_despacho Persistindo o Snapshot Imutável de Coleta e Nome do Estabelecimento
  INSERT INTO public.teles (
    id, tele_code, client_id, status, origin, pickup_address, delivery_address, recipient_name, recipient_phone, notes,
    total_order_amount, delivery_charge, pricing_rule_source, pricing_rule_version,
