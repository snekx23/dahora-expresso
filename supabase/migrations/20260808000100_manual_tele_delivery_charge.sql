-- =====================================================================
-- Dahora Expresso — Migration: Mandatory Manual Delivery Charge (Valor da Tele)
-- Timestamp: 20260808000100
-- File: supabase/migrations/20260808000100_manual_tele_delivery_charge.sql
-- =====================================================================

-- 1. Atualizar public.create_admin_tele para exigir p_delivery_charge > 0
DROP FUNCTION IF EXISTS public.create_admin_tele(UUID, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, NUMERIC, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, BOOLEAN, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, TEXT);
...
BEGIN
  ...
  -- 5. Validar Valor da Tele OBRIGATÓRIO E MAIOR QUE ZERO (Sem fallback para resolve_delivery_charge)
  ...
  -- 8. Normalização do Endereço de Coleta
...
-- 2. Atualizar public.create_client_tele para exigir p_delivery_charge > 0
DROP FUNCTION IF EXISTS public.create_client_tele(TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, NUMERIC, TEXT, TEXT, TEXT, TEXT, TEXT, TEXT, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, BOOLEAN, DOUBLE PRECISION, DOUBLE PRECISION, TEXT, TEXT);
...
BEGIN
  ...
  -- 4. Validar Valor da Tele OBRIGATÓRIO E MAIOR QUE ZERO (Sem fallback para resolve_delivery_charge)
  IF p_delivery_charge IS NULL THEN
    RETURN pg_catalog.jsonb_build_object('success', false, 'error_code', 'INVALID_DELIVERY_CHARGE', 'message', 'Informe um valor válido para a Tele (maior que R$ 0,00).');
  END IF;
  ...
  -- 6. Endereço de Coleta Automático do Cadastro do Cliente Comercial
  ...
  IF v_inserted_tele.id IS NULL THEN
    ...
    RETURN pg_catalog.jsonb_build_object(
      ...
      'version', v_existing_tele.version,
      'created_at', v_existing_tele.created_at,
      'message', 'Solicitação de entrega já processada anteriormente.'
    );
  END IF;
  ...
  VALUES (
    v_inserted_tele.id, 'tele_requested',
    pg_catalog.jsonb_build_object('client_id', v_client_user.client_id, 'source', 'client_portal', 'actor_user_id', v_user_id, 'delivery_charge', v_delivery_charge, 'total_order_amount', v_order_value),
  ...
  VALUES (
    ...
    pg_catalog.jsonb_build_object('client_id', v_client_user.client_id, 'source', 'client_portal', 'delivery_charge', v_delivery_charge, 'total_order_amount', v_order_value),
